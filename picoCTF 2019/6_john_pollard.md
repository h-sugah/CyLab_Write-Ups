...
URL: https://learn.cylabacademy.org/library/6?page=1  
...
「certificates」の部分がリンクになっているので、クリックします。  
ファイル名：「cert」というファイルをダウンロードできます。  
fileコマンドで確認するとPEM形式の証明書ファイルであることが分かります。  
...
opensslコマンドを使い、X.509フォーマットに変換して内容を見てみます。  
...
公開キーのアルゴリズムがrsaEncryptionで、53bitの公開鍵、  
Modulus: 4966306421059967 (0x11a4d45212b17f)  
Exponent: 65537 (0x10001)  
であることが分かります。  

Hintを参照すると、「The flag is in the format picoCTF{p,q}」と書かれており、RSA暗号の n = p * q の関係から p と q を求めれば良いことになります。  
Pythonのsympyモジュールを使用して、素数を導き出します。  
